...
final public class CrSectionizer extends JCasAnnotator_ImplBase {

   static private final Logger LOGGER = Logger.getLogger( "CrSectionizer" );
         ...
         while ( ( line = br.readLine() ) != null ) {
            if ( !line.trim()
               ...
               if ( fields != null && fields.length > 1 && fields[ 0 ] != null
                  ...
                  if ( oldPattern != null ) LOGGER.info(
                        "WARNING: already had a pattern '" + oldPattern + "' for " + id + " which was just replaced by "
                        + p );

                  if ( fields.length > 1 && fields[ 1 ] != null ) {
                     String temp = fields[ 1 ].trim();
                     String previous = section_names.put( id, temp );
                     if ( previous != null ) LOGGER.info( "WARNING: already had a section '" + previous + "' for " + id
                                                          + " which was just replaced by " + temp );
...
//        return Pattern.compile("^(" + sb + ")", patternFlags);
   ...
   static private List<Segment> findSectionsHeadersByPattern( JCas jCas, final String docText ) {
      //////////////////////////////////////////////////////////////////////////
      //        Special for CR where references to papers and cases are made
      String text = docText;
      final int refIndex = text.indexOf( "REFERENCES:" );
      if ( refIndex > 1 ) {
      ...
      for ( String patternName : patterns.keySet() ) {
         ...
         while ( m.find() ) {
            ...
            while ( ( c == '\r' || c == '\n' ) && !isAllWhiteSpace ) {
               ...
               if ( begin > text.length() ) {
                  throw new RuntimeException(
                        "Skipping CRs and LFs at the start of a section heading extended us past the end of the document!?!?"
                        +
            ...
            int last = end - 1;
            c = text.charAt(
                  last );  // heading should be at least one character so can look at character at end-1 safely
            while ( ( c == '\r' || c == '\n' ) && !isAllWhiteSpace ) {
               end--;
               last = end - 1;
               c = text.charAt( last );
               if ( end < segmentHeader.getBegin() ) {
                  throw new RuntimeException(
                        "Skipping CRs and LFs at the end of a section heading took us back before the beginning of the section heading!?!?"
                        +
                        "\nm.start=" + m.start() +
                        "\nm.end=" + m.end() +
...
//            LOGGER.info("Processing segment header: " + header.getId() + " tagText = " + header.getTagText() + " " + header.getBegin() + ", " + header.getEnd());
         int end = cas.getDocumentText()
                      .length(); // handle case for last section
         if ( index + 1 < sectionHeaders.size() ) {
            end = sectionHeaders.get( index + 1 )
                                .getBegin();
         }
...
//                LOGGER.info( "Adding " + segment.getPreferredText() + " " + segment.getId() + " " + segment.getTagText() );
            ...
            LOGGER.info(
                  "Ignoring " + segment.getPreferredText() + " " + segment.getId() + " " + segment.getTagText() + " "
