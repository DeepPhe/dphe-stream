...
import org.healthnlp.deepphe.summary.engine.CrSummaryEngine;
 ...
 * @since 7/14/2020
 */
public enum CrDmsRunner implements Closeable {
   INSTANCE;

   static public CrDmsRunner getInstance() {
      return INSTANCE;
   }

   static private final Logger LOGGER = Logger.getLogger( "CrDmsRunner" );
   ...
   CrDmsRunner() throws ExceptionInInitializerError {
      synchronized ( NLP_PIPER_PATH ) {
         try {
            ...
            Logger.getLogger( "CrDmsRunner" ).error( multE.getMessage() );
   ...
   public PatientSummary createPatientSummary( final String docId, final String text ) {
      try {
         ...
         return CrSummaryEngine.createPatientSummary( patient );
...
//            throw new AnalysisEngineProcessException( multE );
   ...
   public String summarizePatient( final String patientId ) {
      final Patient patient = PatientNodeStore.getInstance().getOrCreate( patientId );
      final PatientSummary patientSummary = CrSummaryEngine.createPatientSummary( patient );
